<div class="h-full relative md:pt-24">

    <div
        class="absolute inset-0 opacity-25 dark:opacity-60"
        :style="color ? 'background: linear-gradient(' + '#' + color.replace('#', '') + ', transparent)' : ''"></div>
    
    <div class="container mx-auto px-4">
        <div class="flex flex-col lg:flex-row py-8 items-start">
            <div class="w-full lg:w-1/3 mx-auto lg:ml-0 z-20">
                ...
                <div class="flex flex-wrap -mx-3 mb-6">
                    <div class="w-full px-3">
                        <x-input wire:model="feed_url" label="Paste your feed URL" />
                    </div>
                </div>
                ...
                @if ($episodes->count() > 0)
                    <div class="flex flex-wrap -mx-3 mb-6">
                        <div class="w-full px-3">
                            <select wire:model="episodeId" label="Episode number or title (leave blank for latest)">
                    ...
                    <div class="flex flex-wrap -mx-3 mb-6">
                        <div class="w-full px-3">
                            <x-input type="color" wire:model="color" label="Choose Color" />
                        </div>
                    </div>
                @endif
            ...
            <div v-if="feed_url && feed_url.length > 0" class="w-full lg:w-1/2">
                ...
                <svg class="absolute opacity-50 z-[-1] bg-texture w-[700px] h-[700px]" viewBox="0 0 156 159" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div class="relative z-5">
                    <label class="block tracking-wide font-bold text-sm mb-2" for="episode">Preview</label>

                    <iframe
                        frameBorder="0"
                        height="180"
                        width="100%"
                        v-bind:src='`${player_url}?color=${color.replace("#", "")}&episode=${episode}&feed=${feed_url}`'></iframe>
                    
                    <div class="pt-5 overflow-hidden mb-5">
                        <label class="block tracking-wide font-bold text-sm mb-2" for="episode">Snippet</label>
                    ...
                    <div>
                        <label class="block tracking-wide font-bold text-sm mb-2" for="episode">Or build one your self dynamically using the URL...</label>
