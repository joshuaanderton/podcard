...
@section('content')
  ...
  <div id="player_builder" class="relative">
    ...
    <div class="flex items-center overflow-hidden min-h-[720px]">

      <div
        class="h-full w-full absolute top-0 left-0 opacity-25"
        :style="color ? 'background: linear-gradient(' + '#' + color.replace('#', '') + ', #ffffff)' : ''"></div>
      
      <div class="container mx-auto px-4">
        <div class="flex flex-col lg:flex-row py-8 items-center">
          <div class="w-full lg:w-1/3 mx-auto lg:ml-0 z-20">

            <div class="pb-6 mb-6 fade-up">
              <h1 class="font-sans font-bold text-black text-4xl sm:text-5xl -mt-5">Podcard</h1>
              <div class="font-sans text-xl sm:text-2xl text-black mt-2">Customize your embeddable player and copy & paste the snippet to any HTML page.</div>
            </div>
          ...
          <div v-if="feed_url && feed_url.length > 0" class="w-full lg:w-1/2">
            ...
            <div class="flex items-center md:justify-end w-full mb-6">
              <a target="_blank" href="https://twitter.com/gettingtoramen" class="text-xs text-gray-700 inline-block bg-white rounded px-4 py-2">
                Made by
