...
<button class="{{ $class ?: '' }} group h-9 w-9 items-center justify-center sm:flex rounded-md bg-black/60" @if ($color) style="color: {{ $color }}" @endif>
  <svg class="h-8 w-8 transition" :class="copied ? 'rotate-[-7deg] text-green-500' : 'group-hover:rotate-[-7deg]'" stroke="currentColor" fill="none" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      ...
      <g :class="copied ? 'opacity-100' : 'opacity-0'">
