...
trait Billable
{
    ...
    {
        if (!$this->stripe_customer_id) return $this;
    ...
    public function card()
    {
        ...
        if (empty($stripe_customer->default_source)) return null;

        foreach ($stripe_customer->sources->data as $source) :
            if ($source->id == $stripe_customer->default_source) return $source;
        endforeach;
    ...
    public function plan()
    {
        if ($this->paypal_subscription_id) :
            ...
            if (in_array($user->id, [4568,4489,4430,5714,4479])) $paypal_amount = 2000;
            if (in_array($user->id, [1805]))                     $paypal_amount = 1400;
            if (in_array($user->id, [731,3703,1585]))            $paypal_amount = 1000;
    ...
    {
        if (!$this->stripe_customer_id) return [];
